...
public:
	...
	bool LoadFromMySQL( // MySQL에서 상점 데이터를 로드한 뒤, 각 아이템 코드에 해당하는 ItemData 정보를 매칭하여 저장
		std::unordered_map<uint16_t, ShopEquipmentItem>& em,
	...
	const ShopEquipmentItem* GetEquipment(uint16_t itemId) const;
...
private:
	...
	std::unordered_map<uint16_t, ShopEquipmentItem> shopEquipmentItemMap;
